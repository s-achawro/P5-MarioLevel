...
def generate_successors(population):
    ...
    elite_count = max(1, len(population) // 10) # number of individuals to keep as-is (top 10%).
    sorted_pop = sorted(population, key=lambda ind: ind.fitness(), reverse=True) #the population sorted from highest fitness to lowest
    elites = sorted_pop[:elite_count]
    results.extend(elites)

    #Tournament selection, randomly select k individuals from the population (k=3).
